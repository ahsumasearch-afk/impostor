...
function claim(code,restore,attempt){
  ...
  p.on("open",()=>{
    ...
    if(restore){
      ...
      H=freshState();
      H.players.push({pid:myPid,name:myName,emoji:myEmoji,color:myColor,score:0,answer:null,vote:null,question:"",online:true,connId:null});
    }
  ...
  p.on("error",e=>{
    ...
    if(e.type==="unavailable-id"){
      ...
      /* Neuer Raum: der zufaellige Code war schon vergeben – den naechsten nehmen. */
      if(!restore){
...
function connectHost(){
  ...
  c.on("data",d=>{
    ...
    if(d.t==="state"){ S=d.s; myQuestion=d.q||""; screen="game"; banner=""; render(); }
